/-!
# Source package model for `chern-conjecture-canonical-lane`
...
def sourceCheckoutHead : String := "634905d6514c8cd084727f0805aef67f08a219de"
def sourceCheckoutClean : Bool := true

def sourceFiles : List SourceFile := [
  { path := "scripts/chn_closure_guard.py", sha256 := "a7e8ccc4b71e33400737cf32ebaade3d703f6f4f650c3c3ad682b560d7abef7f", functionCount := 9, classCount := 0, assignmentCount := 8, parseOk := true },
...
def sourceFunctions : List SourceFunctionDecl := [
  { file := "scripts/chn_closure_guard.py", name := "_finite", args := ["v"], returns := "bool", doc := "", line := 31, isAsync := false },
  { file := "scripts/chn_closure_guard.py", name := "_resolve", args := ["path_str"], returns := "Path", doc := "", line := 35, isAsync := false },
  { file := "scripts/chn_closure_guard.py", name := "_bootstrap_registry", args := ["path"], returns := "None", doc := "", line := 40, isAsync := false },
  { file := "scripts/chn_closure_guard.py", name := "_load_registry", args := ["path"], returns := "dict[str, Any]", doc := "", line := 46, isAsync := false },
  { file := "scripts/chn_closure_guard.py", name := "_load_sigma_from_stitch", args := ["path"], returns := "float | None", doc := "", line := 58, isAsync := false },
  { file := "scripts/chn_closure_guard.py", name := "_entry", args := ["constants", "key"], returns := "tuple[float | None, bool]", doc := "", line := 75, isAsync := false },
  { file := "scripts/chn_closure_guard.py", name := "compute_report", args := ["data", "sigma_star", "strict_coh_zero"], returns := "dict[str, Any]", doc := "", line := 86, isAsync := false },
  { file := "scripts/chn_closure_guard.py", name := "append_history", args := ["path", "report"], returns := "None", doc := "", line := 119, isAsync := false },
  { file := "scripts/chn_closure_guard.py", name := "main", args := [], returns := "None", doc := "", line := 126, isAsync := false },
...
def sourceAssignments : List SourceAssignmentDecl := [
  { file := "scripts/chn_closure_guard.py", name := "SCRIPT_DIR", value := "Path(__file__).resolve().parent", line := 13 },
  { file := "scripts/chn_closure_guard.py", name := "PROJECT_ROOT", value := "SCRIPT_DIR.parent", line := 14 },
  { file := "scripts/chn_closure_guard.py", name := "DEFAULT_REGISTRY", value := "\"artifacts/constants_registry.json\"", line := 16 },
  { file := "scripts/chn_closure_guard.py", name := "DEFAULT_STITCH", value := "\"artifacts/stitch_constants.json\"", line := 17 },
  { file := "scripts/chn_closure_guard.py", name := "DEFAULT_OUT", value := "\"repro/certificate_runtime.json\"", line := 18 },
  { file := "scripts/chn_closure_guard.py", name := "DEFAULT_HISTORY", value := "\"repro/drift_guard_runs.jsonl\"", line := 19 },
  { file := "scripts/chn_closure_guard.py", name := "PRIMARY_KEYS", value := "[\"kappa_canonical\", \"sigma_deformation\", \"kappa_compact\", \"rho_rigidity\", \"chern_transfer\"]", line := 21 },
  { file := "scripts/chn_closure_guard.py", name := "COHERENCE_KEY", value := "\"eps_coh\"", line := 28 },
...
def constantSpecs : List ConstantSpec := [
  { group := "constants", key := "kappa_canonical", status := "derived_numeric", formula := "c_canonical_raw * canonical_density_raw - e_canonical_raw", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A", validation := "required_positive", componentKeys := ["c_canonical_raw", "canonical_density_raw", "e_canonical_raw"] },
  { group := "constants", key := "sigma_deformation", status := "derived_numeric", formula := "deformation_floor_raw - connection_loss_raw - restart_loss_raw", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B", validation := "required_positive", componentKeys := ["connection_loss_raw", "deformation_floor_raw", "restart_loss_raw"] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D", validation := "required_positive", componentKeys := ["rho_rigidity_raw"] },
  { group := "constants", key := "chern_transfer", status := "derived_numeric", formula := "c_transfer_raw * transfer_gain_raw - e_transfer_raw", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4", validation := "required_positive", componentKeys := ["c_transfer_raw", "e_transfer_raw", "transfer_gain_raw"] },
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Appendix E.6", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Section 5 / stitch constants", validation := "required_positive", componentKeys := ["sigma_star_can_raw"] }
]

def registryConstants : List RegistryConstant := [
  { key := "chern_transfer", value := "1.0305400000000002", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#chern_transfer", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4" },
  { key := "eps_coh", value := "0.0", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#eps_coh", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Appendix E.6" },
  { key := "kappa_canonical", value := "1.093874", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_canonical", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A" },
  { key := "kappa_compact", value := "0.8051529790660226", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compact", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C" },
  { key := "rho_rigidity", value := "1.077", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#rho_rigidity", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D" },
  { key := "sigma_deformation", value := "1.075", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_deformation", sourceSection := "paper/CHERN_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B" }
]
